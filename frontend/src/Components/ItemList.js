...
const ItemList = ({ userdata = ""}) => {
    ...
    const productList = productLists?.filter(product => (product?.sender!==deletedProduct)&&(product?.status===published||userdata === "" || product?.sender === userdata?.email||user?.title===admin))
    let list;
    useEffect(() => {
        if (!productLists) {
